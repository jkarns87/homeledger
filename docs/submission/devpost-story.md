...
- The required-check list in "How I built it" is mirrored from `docs/RUNBOOK.md` §8.3, which is the only place that states it authoritatively. Branch protection is a repository setting no check can validate, and this list has gone stale once already — if §8.3 changes, change it here in the same pass.
- 2026-09-21: Accomplishments re-based on smoke run 35616643237, which is the run *after* a live teardown and bring-up, so the numbers now carry that context. The round trip itself and the identifier-rotation finding moved up; a new Challenges paragraph carries the latter. Nothing moved down. The PENDING list is unchanged — Bedrock is still blocked, the load smoke still has not been run, and the simulator and Ring pipeline still do not exist.
...
**CI builds the artifact, not just the code, and that distinction was paid for.** The test job installs the whole pnpm workspace; the container copies only two packages into its build context, so the two disagree about what the workspace contains and the tests can be green for a workspace edge the image cannot resolve. Three merges passed `test` and then failed the deploy's own image build before anyone noticed. There is now an `image` job that builds the same Dockerfile the deploy builds, on native arm64 because that is what AgentCore runs, so the disagreement fails a pull request instead of a merge. It holds no AWS credentials and pushes nothing — it needs none, and a pull-request-triggered job is the wrong place to hand them out. `main` requires three checks to merge — `test`, `terraform`, `image` — and all three deliberately live in the one workflow that has no path filter, so every pull request can satisfy them. The Terraform `plan` job is *not* among them, and that is the second lesson from the same family: it only triggers on certain paths, so requiring it meant a documentation-only pull request waited forever on a check that would never arrive. A required check that never reports doesn't fail, it blocks.
...
**Blast radius is not consequence radius, and every safety mechanism I built measured the wrong one.** The hackathon expects resources to be live only during test windows, so I built a dispatch-only teardown for the AgentCore runtime and spent most of that effort on scope. The scope is genuinely tight, and structurally rather than by policy: the runtime is the only conditional resource in the entire configuration, so an apply with an empty image URI *cannot* express a wider destroy however it is asked. On top of that the job plans first, reads the plan back as JSON, and refuses to apply if it would create anything or delete anything but the runtime's exact address. Then I ran it. It behaved perfectly — one resource destroyed, Cognito, the Knowledge Base, the table, the images and the data all untouched — and it invalidated every invocation URL I had ever issued, because AgentCore generates the runtime's id at create time as a random suffix on the ARN, and the URL is that ARN encoded into a path. I went looking for the alias layer. Named endpoints exist, and the `?qualifier=DEFAULT` in the URL is an endpoint name rather than a version, so for a while it looked like the answer. The endpoint ARN pattern settles it in one line: `.../runtime/<name>-[a-zA-Z0-9]{10}/runtime-endpoint/<name>`. The endpoint is a *child* of the runtime. It inherits the id that rotates, and dies with its parent. Across 170-odd control-plane commands there is no alias, no custom domain, no DNS binding — the only real indirection is a Gateway, which is not a transparent proxy and would very likely break the elicitation this project is built around. The generalisation is the sharper sibling of the one above: **a change can touch exactly one resource and still break every consumer downstream of an identifier that resource owns.** Blast radius counts resources; consequence radius counts addresses, and the two are not related. The question a review has to ask beside "what does this destroy" is "what *names* does this destroy, and who is holding them" — and for any platform-generated id, whether the platform offers an alias layer at all, before you hand the id to anyone. The fix was documentation, because there is no code fix: the warning now sits in the teardown workflow's confirmation prompt, where the person about to pull the lever reads it.
...
The booking flow is the one I would show first, and it runs on both protocol generations against the deployed runtime. In smoke run 35616643237, a 2025-era client initializes a session through AgentCore, and `book_service` completes three elicitation rounds — provider, arrival window, confirmation — emits progress `0` through `3`, and writes a real visit (`Kettle Creek Water Heaters`, `visit_eivz3iyo4k6eujkw`) in 1807 ms. `get_visit` reads the same visit back in 308 ms. A 2026-07-28 client lists and calls tools statelessly on the same endpoint in the same run, and the same handler serves both — no transport-specific branch in any tool.

The numbers from that run: cold connect 936 ms modern, 948 ms legacy. `tools/list` 709 ms, `list_appliances` 743 ms modern and 371 ms legacy, `maintenance_due` 734 ms, `ask_manual` 837 ms, `echo_confirm` with its elicitation round trip 665 ms, `get_visit` 308 ms — every single-round call under a second against a 3-second budget. `book_service` is the one call not held to that budget, and deliberately so: it spans three human round trips and a deliberate availability delay.

**That run is worth reading with its context: it is the one taken immediately after tearing the whole runtime down and putting it back.** Teardown destroyed exactly one resource in 4 m 48 s (`0 added, 0 changed, 1 destroyed`); bring-up recreated it in **5 seconds** by redeploying an image already sitting in ECR, rather than rebuilding; the smoke above then passed against the new runtime. The Cognito client id, the Knowledge Base id, the DynamoDB table and its seeded household all came through byte-identical. So the demo can be parked and restored on demand, and the restore is seconds rather than a fresh deploy — which is what makes "live only during test windows" a real operating mode rather than an intention.

**The runtime also survives ordinary deploys without moving.** Terraform replaces this resource on exactly two changes — a rename, or an artifact-type flip between container and code — so image pushes, environment changes and role changes all update in place and keep the same ARN. Confirmed live at `0 to add, 1 to change, 0 to destroy` on an image bump, and by an earlier runtime that reached its fourteenth in-place version on one unchanged ARN.
...
Write handlers for the newest protocol and let the SDK shim the past; the reverse doesn't work. Treat every "coming soon" in a platform's docs as a feature flag with a fixture behind it. Ask how far a capability dependency reaches, not merely which operation needs it — the honest answer is usually further than the first failing call suggests. Ask the same question about identifiers: before handing out an address, find out whether the platform can ever give you the same one back, because blast radius and consequence radius are different quantities and only one of them shows up in a plan. Rehearse the operational paths — teardown, bring-up, restore — while a mistake is still cheap; the first execution of a path should never be the one that matters. Put the friction log in the repo on day one, because the decisions it drove are the architecture.
...
`ask_manual` has never retrieved a real passage: the Knowledge Base is provisioned but unqueryable under the account's Bedrock block, and where no Knowledge Base is configured the tool serves a small in-memory fixture set. No Ring integration exists in the repository — `recent_events` returns door and sensor rows only if something writes them, and nothing does yet. `get_visit`'s `snapshotUrl` is always null. There is no simulator; `apps/` contains the MCP server and the stdio bridge that lets Claude Code reach it, and nothing else.
...
**Simulator** (How I built it, new paragraph; built in Plan 3 — `apps/simulator` — but not yet verified: the one run this needs, the deployed runtime with the real model, has not been executed by anyone. Verified only when that run happens; tracked as `FRICTION-LOG.md` FL-055, which carries the checklist it has to satisfy. Nothing here moves up before then, including this corrected description — the design spec's Strands-on-Bedrock agent was itself never built, so the PENDING text was already describing the wrong thing to verify)
> **Simulated Alexa+ client.** The MCP Toolkit is a private preview and the organizers confirmed entrants cannot call Alexa+ itself, so the interaction layer is a Next.js web client shaped like an Echo Show 8 (768×480 canvas at 1.667 scale, Alexa+'s published card colors) driving the deployed MCP server through an agent on the Anthropic API, not a Strands agent on Bedrock. Bedrock model invocation is blocked account-wide on this account — the same block `ask_manual` is behind — so a Bedrock-backed agent could not make one call; the Anthropic API sidesteps the block entirely and is the same path Claude Code already uses to drive this server. Everything else in the original design stands: an MCP client over the documented transport, elicitation rendered as a card rather than chat text, and a debug drawer showing the JSON-RPC traffic (each method sent, its status and timing, and the elicitation and progress messages arriving, capped at the last 100 lines), the negotiated protocol version, the session ID, and per-call timing.
...
Tags: Anthropic Claude, Next.js, React. No separate try-it-out link: the simulator runs locally against the deployed server rather than being hosted (Amplify hosting is out of scope for Plan 3 — a public endpoint holding an API key is its own security decision, not a build step).
