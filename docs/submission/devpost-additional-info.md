...
The runtime's lifecycle has been exercised, not just deployed. Terraform updates it **in place** on an image, environment-variable or execution-role change — the provider replaces it on exactly two things, a rename and an artifact-type flip between container and code — so an ordinary merge to `main` moves the container URI and keeps the same ARN (observed at `Plan: 0 to add, 1 to change, 0 to destroy`, and on an earlier runtime that reached its fourteenth in-place version without the ARN moving). A third workflow parks the demo between test windows: dispatch-only, gated on a typed confirmation, and structurally incapable of destroying anything but the runtime, because the runtime's `count` is the only conditional in the configuration and the job additionally reads its own plan back as JSON and refuses to apply if it would create anything or delete any other address. Both modes have been run against the live account — teardown destroyed one resource in 4 m 48 s, bring-up recreated it in **5 seconds** by redeploying an image already in ECR, and the smoke passed afterwards with the Cognito client id, the Knowledge Base id and the seeded table all unchanged. The one thing that does not survive is the runtime's own id, which AgentCore generates at create time and which no alias, qualifier or custom domain in the service can insulate a caller from — the third finding in the friction log from this work (FL-038), and the reason the setup helper discovers the runtime by name through `ListAgentRuntimes` rather than storing an ARN.
...
**What.** HomeLedger is a new public repository, MIT licensed, created on 2026-09-13 inside the hackathon window. It is a Streamable HTTP MCP server in TypeScript with a pnpm monorepo: `packages/core` (domain schemas, DynamoDB single-table repository with an in-memory twin and a shared contract suite, the Bedrock retrieval adapter), `apps/mcp-server` (the server, nine tools, resources, four MCP Apps widgets, prompt, container), `apps/mcp-bridge` (a stdio-to-Streamable-HTTP bridge that mints a Cognito token and discovers the runtime by name, so Claude Code can reach the deployed endpoint), and `scripts` (deployed smoke, seeding, manuals ingestion).
...
15. **AgentCore Runtime: an alias, or a runtime endpoint whose ARN does not embed the runtime's generated id.** A runtime's id is generated at create time as a random ten-character suffix on its ARN, and the invocation URL is that ARN percent-encoded into a path — so destroying and recreating a runtime, which is the documented way to park a demo between test windows, invalidates every URL ever issued. Named endpoints look like the escape hatch and are not: `agentRuntimeEndpointArn` is patterned `.../runtime/<name>-[a-zA-Z0-9]{10}/runtime-endpoint/<name>`, so an endpoint is a child of the runtime, inherits the rotating suffix, and is a different resource after a recreate. Nothing else in the 170-plus `bedrock-agentcore-control` commands is alias- or domain-shaped, and Gateway is a tool-reexposing façade rather than a transparent proxy, so it is not a substitute for a stateful MCP server that depends on multi-round elicitation. Lambda has `create-alias`; AgentCore needs the equivalent. Priority: Important. (FL-038)
16. **AWS Support: let an account read its own case status on the Basic plan.** The Support API requires Business or Enterprise, so an account blocked by new-account verification — precisely the population most likely to hit it — can verify the symptom from CI indefinitely but must open the console to learn whether the cause has moved. A read-only `describe-cases` for one's own cases would make the loop automatable. Priority: Nice-to-have. (FL-019)
...
- **AgentCore**: the CLI has no TypeScript MCP path (FL-008); the protocol contract cites a superseded MCP revision (FL-009). And there is no stable address for a runtime: the id is generated at create time and lives inside the invocation URL, named endpoints are children of the runtime and inherit it, and nothing across 170-plus control-plane commands is alias- or domain-shaped — so the documented practice of tearing a demo down between test windows silently invalidates every URL handed out (FL-038).
